...
import { useDesktopUIStore, useFlightStore, useFlightMode } from '@/stores';
...
export function UnifiedSidebar() {
    ...
    // Flight dashboard state
    const dashboardOpen = useFlightStore((state) => state.dashboardOpen);
    const toggleDashboard = useFlightStore((state) => state.toggleDashboard);
    ...
    // Flight button component - toggles tactical dashboard
    const FlightButton = () => (
        <Button
            variant={dashboardOpen || flightMode !== 'off' ? 'default' : 'outline'}
            size="icon"
            onClick={toggleDashboard}
            title="Tactical Flight Dashboard"
            className={`fixed bottom-6 right-4 shadow-2xl w-14 h-14 rounded-2xl select-none transition-all ${
                flightMode === 'manual'
                    ...
                    : flightMode === 'autopilot'
                        ...
                        : dashboardOpen
                            ...
                            : 'bg-white/90 backdrop-blur-xl text-gray-800 hover:bg-white border border-white/50'
            }`}
            style={{ zIndex: Z_INDEX.CONTROLS }}
        >
